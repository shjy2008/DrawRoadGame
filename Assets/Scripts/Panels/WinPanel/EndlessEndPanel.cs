...
public class EndlessEndPanel : BasePanel
{
    public static EndlessEndPanel instance;
    ...
    protected override void OnOpenPanel()
    {
        ...
        IEnumerator _()
        {
            ...
            PlayerData.TryGainNewRoad_EndlessScore();
        }
        StartCoroutine(_());

        // Title
        gameObject.transform.Find("Bg/Title").GetComponent<Text>().text = string.Format("SCORE: {0}", m_score);
        gameObject.transform.Find("Bg/BestScore").GetComponent<Text>().text = string.Format("BEST: {0}", PlayerData.bestScore);
        ...
        if (m_isNewRecord)
        {
            ...
            // Open action
            gameObject.transform.Find("Bg").GetComponent<Animator>().Play("WinPanel_Open");

            gameObject.transform.Find("Bg/NewRecord").gameObject.SetActive(true);
        }
        else
        {
            gameObject.transform.Find("Bg/NewRecord").gameObject.SetActive(false);
        }
    }
    ...
    {
        m_score = score;
        m_isNewRecord = score > PlayerData.bestScore;
