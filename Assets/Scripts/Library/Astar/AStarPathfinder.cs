...
namespace AStar
{
	public class AStarPathfinder
	{
		...
		{
			int MAP_WIDTH = s_map.GetLength(0);
			int MAP_HEIGHT = s_map.GetLength(1);
		...
		public static void SetMap(int[,] mapArray)
		{
			s_map = mapArray;
		}

		// really!!! find!!! path!!! 
		public List<NodePosition> Pathfind(NodePosition startPos, NodePosition goalPos)
		{
			// Reset the allocated MapSearchNode pointer
			InitiatePathfind();

			// Create a start state
			MapSearchNode nodeStart = AllocateMapSearchNode(startPos);

			// Define the goal state
			MapSearchNode nodeEnd = AllocateMapSearchNode(goalPos);
	...
	public class MapSearchNode
	{
		...
		public bool ValidNeigbour(int xOffset, int yOffset)
		{
			...
			return (AStarPathfinder.GetMap(position.x + xOffset, position.y + yOffset) < 1);
		}
		...
		public float GetCost(MapSearchNode successor)
		{
			...
			return AStarPathfinder.GetMap(successor.position.x, successor.position.y);
		}
